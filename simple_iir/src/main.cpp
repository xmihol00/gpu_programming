...
struct DeviceSignals 
{
	...
	DeviceSignals(ConfigLoader & loader)
	{
		...
		for (int i = 0; i < num; ++i)
		{
			...
			_inputs.push_back(s.get());
			_outputSignals.emplace_back(Signal(s.length(), 0.0f));
			totalMemory += s.length();
			_signalLengths.push_back(s.length());
		}
		
		totalMemory *= sizeof(float);
		allocateMemoryOnDevice(_d_inputs, totalMemory);
		allocateMemoryOnDevice(_d_outputs, totalMemory);
	}

	~DeviceSignals()
	{
		freeMemoryOnDevice({ _d_inputs, _d_outputs });
	}

	void transferFromCPU() 
	{
		copySignalsToDevice(_inputs, _signalLengths, _d_inputs);
	}
	...
	void transferToCPU() 
	{
		...
		for (size_t i = 0; i < _outputSignals.size(); i++)
		{
			...
			copySignalFromDevice(_d_outputs, signal, _signalLengths[i], offset);
			offset += _signalLengths[i];
		}
	}
